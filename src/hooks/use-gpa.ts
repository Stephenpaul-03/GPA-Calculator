...
export function calculateGPA(
  subjects: Subject[]
): { results?: GPAResults; error?: GPAError } {
  if (!subjects || subjects.length === 0) {
    return {
      error: {
        message: "No subjects found. Please add subjects before calculating."
      }
    }
  }
  ...
  if (missingDataSubjects.length > 0) {
    return {
      error: {
        message: "Missing data in subjects",
        warnings: missingDataSubjects.map(i => ({
          subjectIndex: i,
          message: "Missing credits or grade"
        }))
      }
    }
  }
  ...
  if (invalidCreditsSubjects.length > 0) {
    return {
      error: {
        ...
        warnings: invalidCreditsSubjects.map(i => ({
          subjectIndex: i,
          message: "Credits must be between 0 and 6"
        }))
      }
    }
  }
  ...
  const semesterResults: SemesterResult[] = Object.entries(
    semesterMap
  ).map(([sem, data]) => ({
    semester: sem,
  ...
  semestersSorted.forEach(sem => {
    ...
    cgpaTrend.push({
      semester: sem,
      cgpa: (runningScore / runningCredits).toFixed(3)
    })
  })
  ...
  const sortedSemesterGPA = semesterResults
    .slice()
    .sort((a, b) => parseFloat(b.gpa) - parseFloat(a.gpa))

  const maxSemesterGPA = {
    semester: sortedSemesterGPA[0].semester,
    gpa: sortedSemesterGPA[0].gpa
  }

  const minSemesterGPA = {
    semester:
      sortedSemesterGPA[sortedSemesterGPA.length - 1].semester,
    gpa:
      sortedSemesterGPA[sortedSemesterGPA.length - 1].gpa
  }
